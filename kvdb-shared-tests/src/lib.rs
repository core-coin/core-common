...
pub fn test_iter_with_prefix(db: &dyn KeyValueDB) -> io::Result<()> {
	...
	// empty prefix
	let contents: Vec<_> = db.iter_with_prefix(0, b"").map(Result::unwrap).collect();
	...
	// prefix a
	let contents: Vec<_> = db.iter_with_prefix(0, b"a").map(Result::unwrap).collect();
	...
	// prefix abc
	let contents: Vec<_> = db.iter_with_prefix(0, b"abc").map(Result::unwrap).collect();
	assert_eq!(contents.len(), 2);
	assert_eq!(&*contents[0].0, key3);
	assert_eq!(&*contents[1].0, key4);

	// prefix abcde
	let contents: Vec<_> = db.iter_with_prefix(0, b"abcde").map(Result::unwrap).collect();
	assert_eq!(contents.len(), 0);

	// prefix 0
	let contents: Vec<_> = db.iter_with_prefix(0, b"0").map(Result::unwrap).collect();
